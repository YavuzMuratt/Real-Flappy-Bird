...
pygame.init()
pygame.display.set_caption("Zıpzıp Kuş")
...
class Bird:
    ...
    def draw(self, win):
        ...
        # pygame.draw.line(win, (255, 0, 0), (self.x, self.y), (self.x + self.tilt, self.y), 5)
    ...
    while run:
        ...
        for pipe in pipes:
            if pipe.collide(bird):
                draw_text = STAT_FONT.render("Game Over", 1, (255, 255, 255))
        ...
        if bird.y + bird.img.get_height() >= 850:
            draw_text = STAT_FONT.render("Game Over", 1, (255, 255, 255))
            win.blit(draw_text, (SCREEN_WIDTH / 2 - draw_text.get_width() / 2, SCREEN_HEIGHT / 2 - draw_text.get_height() / 2))
            pygame.display.update()
            pygame.time.delay(3000)
